...
function SettingsPanel({ profile, preferences, onClose, onSaveProfile, onSavePreferences, onPasswordChange, onSignOut, onDeleteAccount }: { profile: Profile; preferences: EmailPreferences; onClose: () => void; onSaveProfile: (profile: Profile) => Promise<void>; onSavePreferences: (preferences: EmailPreferences) => Promise<void>; onPasswordChange: (currentPassword: string, newPassword: string) => Promise<string>; onSignOut: () => Promise<void>; onDeleteAccount: () => Promise<void> }) {
  ...
  async function confirmDeleteAccount() {
    ...
    try {
      ...
      setDeleteMessage(error instanceof Error ? error.message : "Não foi possível excluir sua conta.");
      setDeleting(false);
    }
  }
  ...
  return <div className="fixed inset-0 z-[60] flex justify-end bg-black/40 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="settings-title"><button className="absolute inset-0" onClick={onClose} aria-label="Fechar configurações" /><section className="app-scrollbar relative h-full w-full max-w-[680px] overflow-y-auto bg-[#fbfaf7] shadow-2xl dark:bg-[#171310]"><header className="sticky top-0 z-10 flex items-center justify-between border-b border-stone-200 bg-[#fbfaf7]/95 px-5 py-5 backdrop-blur-xl sm:px-7 dark:border-white/8 dark:bg-[#171310]/95"><div><p className="text-[10px] font-black uppercase tracking-[.15em] text-red-700 dark:text-amber-400">Minha conta</p><h2 id="settings-title" className="mt-1 text-2xl font-black tracking-tight">Configurações</h2></div><button onClick={onClose} className="grid size-10 place-items-center rounded-xl border border-stone-200 bg-white dark:border-white/8 dark:bg-white/5" aria-label="Fechar"><X className="size-5" /></button></header><div className="p-5 sm:p-7"><nav className="grid grid-cols-3 gap-1 rounded-xl bg-stone-100 p-1 dark:bg-white/5" aria-label="Seções das configurações">{[{id:"profile" as const,label:"Perfil",icon:CircleUserRound},{id:"security" as const,label:"Segurança",icon:ShieldCheck},{id:"preferences" as const,label:"E-mails",icon:MailCheck}].map(({id,label,icon:Icon})=><button key={id} type="button" onClick={() => setTab(id)} className={`flex items-center justify-center gap-2 rounded-lg px-2 py-2.5 text-xs font-extrabold transition ${tab===id?"bg-white text-red-700 shadow-sm dark:bg-[#2a2521] dark:text-amber-300":"text-stone-500 dark:text-stone-400"}`}><Icon className="size-4" /><span className="hidden sm:inline">{label}</span></button>)}</nav>
      ...
      </div></section>{deleteConfirmOpen && <div className="fixed inset-0 z-[80] grid place-items-center bg-black/45 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="delete-account-title"><button type="button" className="absolute inset-0" onClick={() => !deleting && setDeleteConfirmOpen(false)} aria-label="Fechar confirmação" /><section className="relative w-full max-w-md rounded-[24px] bg-[#fbfaf7] p-6 shadow-2xl dark:bg-[#211c18]"><div className="flex items-start gap-3"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-red-100 text-red-700 dark:bg-red-500/10 dark:text-red-300"><Trash2 className="size-5" /></span><div><h3 id="delete-account-title" className="text-xl font-black">Excluir conta?</h3><p className="mt-2 text-sm leading-6 text-stone-500 dark:text-stone-400">Essa ação é permanente e apagará seu perfil, treinos, notas e plano de jogo.</p></div></div>{deleteMessage&&<p role="alert" className="mt-4 rounded-xl bg-red-50 p-3 text-xs font-bold text-red-700 dark:bg-red-500/10 dark:text-red-300">{deleteMessage}</p>}<div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={() => setDeleteConfirmOpen(false)} disabled={deleting} className="rounded-xl border border-stone-200 px-4 py-3 text-sm font-bold disabled:opacity-50 dark:border-white/10">Cancelar</button><button type="button" onClick={confirmDeleteAccount} disabled={deleting} className="flex items-center justify-center gap-2 rounded-xl bg-red-700 px-4 py-3 text-sm font-extrabold text-white disabled:opacity-60"><Trash2 className="size-4" />{deleting ? "Excluindo..." : "Excluir conta"}</button></div></section></div>}</div>;
}
